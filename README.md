...
源码目录，工作流类型、技术栈和验证信息通过 `case.yaml` 表达。

这些项目用于回答一个具体问题：ZaoFu 能否把需求讨论、PRD、实现、验证和证据
收口连续交付为可运行产品，而不只是生成代码片段。这些案例面向真实领域问题，
并参考相关 GitHub 开源项目的应用场景、领域模型和工程实践。ZaoFu 基于各项目
PRD 重新设计并实现产品功能、交互体验和验证闭环，同时按需使用经过来源核验的
开源数据集、仿真环境和测试语料。每个项目的 `README.md` 说明开发目的、参考项目
与数据来源，`CASE.md` 和 `case.yaml` 记录交付过程与快照来源。
