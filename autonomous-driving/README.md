...
自动驾驶领域的 ZaoFu 完整项目案例。当前案例覆盖六条互补链路：
...
| [ScenarioForge v2](scenarioforge-v2/CASE.md) | 如何统一创作并在 MetaDrive/SMARTS 执行多参与者场景 | Python + Three.js 双后端本地场景工作台 | 多轮 PRD/Issue 恢复后收口统一创作、隔离执行和跟车回放 | 锁定依赖、合同/真实后端/浏览器测试与许可证边界复核 |
...
| [HighwayPilot Lab](highwaypilot-lab/CASE.md) | 如何在施工并道场景中完成三维驾驶决策、策略比较和 Episode 回放 | Python + Three.js 本地驾驶决策实验室 | 场景配置、Gymnasium、WebSocket HMI、策略评估和回放能力收口 | construction-v0、Seed 复现、Python/前端/WebSocket 测试与模型合同 |
