...
Writer `allowed_paths` are writable scopes and must not overlap across tasks,
including tasks separated by dependency edges or waves. Do not add a
sibling-owned file to another task's `allowed_paths` merely because its tests
read that file; the plan-wide verification scope already permits read-only
references to sibling-owned paths. Pre-submit validation applies the same path
...
validation-only regression file, normally add it to `allowed_paths`, omit it
from `exclusive_files`, and state in `explicit_non_goals` that it must not be
modified. Never omit a command operand merely because it is "validation-only";
that produces a mechanically invalid first plan and wastes a rework turn.
