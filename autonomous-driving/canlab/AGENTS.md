...
- `project.state_dir` 当前解析为 `.zf-zaofu-can-lab`;这是运行态目录,不是源码。
...
> Browser-based CAN protocol exploration lab using only synthetic and offline data. P0 covers a demo DBC browser, deterministic CAN replay, traceable decoding, virtual vehicle instruments, tests, and an explicit no-hardware-transmit boundary.
