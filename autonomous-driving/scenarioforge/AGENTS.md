...
- `project.state_dir` 当前解析为 `.zf-scenarioforge-case2-full-20260802t133411z`;这是运行态目录,不是源码。
...
- 项目名: `scenarioforge-case2-p0-completion-20260808`

### 背景与目标

> Close the remaining ScenarioForge MetaDrive P0 product gaps with real samples
<!-- ZF:PROJECT-CONTEXT:END -->

<!-- ZF:PROFILE:START -->
## 探测到的项目栈 (managed by zf profile)

- layout: single · confidence: high
- `.` — python · backend · 有测试
  - test: `pytest`
- `web` — node (react, react-dom, vite) · frontend · 有测试
  - build: `npm run build`
  - test: `npm test`

- 建议 gate: ruff check ., pytest, npm run typecheck, npm test
<!-- ZF:PROFILE:END -->
